...
export default function HomePage() {
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#F4F5F7] flex flex-col font-sans overflow-x-hidden">
      {/* Ticker */}
      <div className="bg-emerald-600 text-white text-[11px] font-bold flex items-center overflow-hidden h-7 shrink-0 sticky top-[60px] z-30">
        <div className="shrink-0 bg-emerald-800 px-3 h-full flex items-center gap-1.5 uppercase tracking-wider">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 p-3">
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-3">
          ...
          {/* Hero ASEAN Map */}
          <div className="relative bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-lg" style={{ minHeight: 420 }}>
            <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10 pointer-events-none">
              <div className="flex items-center gap-2 bg-white/90 backdrop-blur px-3 py-2 rounded-xl border border-gray-200 shadow-sm">
                <Globe className="w-4 h-4 text-emerald-600" />
                <span className="text-gray-900 font-bold text-sm">{t("home.aseanMarketIntel")}</span>
                <span className="bg-emerald-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded ml-1 animate-pulse">{t("home.live")}</span>
              </div>
              <div className="flex items-center gap-1.5 bg-white/90 backdrop-blur px-3 py-1.5 rounded-xl border border-gray-200 shadow-sm text-[10px] font-bold text-gray-600">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> {t("home.marketsTracked")}
              </div>
            </div>
              ...
              <ZoomableGroup zoom={1} center={[117, 3]} maxZoom={6}>
                    ...
                    geographies.map((geo) => {
                      ...
                      return (
                        <Geography
                          key={geo.rsmKey}
                          geography={geo}
                          fill={isASEAN ? "#10B981" : "#E5E7EB"}
                          stroke={isASEAN ? "#059669" : "#D1D5DB"}
                          strokeWidth={isASEAN ? 0.8 : 0.3}
                          style={{
                            default: { outline: "none" },
                            hover: { fill: isASEAN ? "#059669" : "#D1D5DB", outline: "none" },
                    ...
                    geographies.map((geo) => {
                      ...
                      return (
                        <Marker key={`lbl-${geo.rsmKey}`} coordinates={centroid as [number, number]}>
                          <text textAnchor="middle" style={{ fontFamily: "Inter", fontSize: "5px", fill: "#065F46", fontWeight: "bold", pointerEvents: "none", textTransform: "uppercase" }}>
                            {geo.properties.name}
                          </text>
                        </Marker>
                      );
                ...
                {ASEAN_MARKERS.map(({ name, coordinates, color, offset }) => (
                  <Marker key={name} coordinates={coordinates as [number, number]}>
                    ...
                    <text textAnchor="middle" y={offset} style={{ fontFamily: "Inter", fontSize: "6px", fill: "#064E3B", fontWeight: "bold" }}>
                      {name}
                    </text>
                  </Marker>
                ))}
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-3 bg-white/90 backdrop-blur px-4 py-2 rounded-lg border border-gray-200 shadow-sm text-[9px] uppercase font-bold text-gray-500 pointer-events-none">
              <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-emerald-600 border border-emerald-500" /> {t("home.aseanActive")}</div>
              <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-emerald-400" /> {t("home.msmeHub")}</div>
              <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-gray-300" /> {t("home.nonAsean")}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {QUICK_LINKS.map(({ href, labelKey, icon: Icon, color, descKey }) => (
              <Link key={href} href={href}
                className="group flex flex-col gap-2 bg-white hover:bg-gray-50 border border-gray-200 hover:border-emerald-300 rounded-xl p-3 transition-all duration-200 hover:-translate-y-0.5 shadow-sm"
              >
                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${color} flex items-center justify-center shadow-sm`}>
                  <Icon className="w-4 h-4 text-white" />
                </div>
                <div>
                  <p className="text-gray-900 font-bold text-xs">{t(labelKey)}</p>
                  <p className="text-gray-500 text-[10px] mt-0.5">{t(descKey)}</p>
                </div>
                <ArrowUpRight className="w-3 h-3 text-gray-400 group-hover:text-emerald-500 self-end -mt-1 transition-colors" />
              </Link>
            ))}
          </div>

          {/* Live News */}
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden flex flex-col shadow-sm" style={{ height: 280 }}>
            <div className="bg-gray-50 border-b border-gray-200 px-4 py-2 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-900 text-sm">{t("home.liveNews")}</h3>
              ...
              <div className="flex gap-1 text-[10px] font-bold">
                {Object.keys(NEWS_STREAMS).map((n) => (
                  <button key={n} onClick={() => setActiveNews(n)}
                    className={clsx("px-2 py-1 rounded transition", activeNews === n ? "bg-red-500 text-white" : "text-gray-500 hover:bg-gray-100 hover:text-gray-900")}
                  >{n}</button>
                ))}
              </div>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-3">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-gray-900 font-bold text-xs uppercase flex items-center gap-1.5">
                <ShieldAlert className="w-3.5 h-3.5 text-emerald-600" /> {t("home.aseanRisk")}
              </h3>
              <span className="text-[9px] font-bold text-emerald-600 border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 rounded uppercase">{t("home.live")}</span>
            </div>
            <div className="space-y-2.5">
              {ASEAN_RISK.map(({ country, flag, risk, trend, color }) => (
                <div key={country}>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-gray-700">{flag} {country}</span>
                    <div className="flex items-center gap-1">
                      <span className="text-xs font-black text-gray-900">{risk}</span>
                      {trend === "rising" && <TrendingUp className="w-3 h-3 text-red-500" />}
                      {trend === "stable" && <div className="w-3 h-0.5 bg-gray-400 rounded-full" />}
                    </div>
                  </div>
            ...
            <div className="mt-4 flex items-center gap-3 bg-gray-50 rounded-xl p-3 border border-gray-200">
              <div className="relative w-14 h-14 flex-shrink-0">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                  <path strokeWidth="3.5" stroke="#E5E7EB" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-lg font-black text-gray-900">{globalRisk}</span>
                </div>
              </div>
              <div>
                <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider">{t("home.compositeRisk")}</p>
                <p className="text-sm font-bold text-emerald-600 mt-0.5">{t("home.lowStable")}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-3 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[10px] text-gray-500 font-bold uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {ASEAN_WEBCAMS.map((cam, i) => (
                <div key={i} className="relative aspect-video bg-gray-100 rounded-lg overflow-hidden border border-gray-200 pointer-events-none">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 flex flex-col gap-2 shadow-sm">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-gray-900 uppercase flex items-center gap-1.5">
                <Zap className="w-3.5 h-3.5 text-emerald-600" /> {t("home.aiMarketBrief")}
              </h3>
              <span className="text-[9px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200 px-1.5 py-0.5 rounded">{t("home.live")}</span>
            </div>
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3">
              <p className="text-[11px] text-emerald-800 font-medium leading-relaxed">{liveNewsBrief}</p>
            </div>
            <div className="grid grid-cols-3 gap-1.5 mt-1">
              {[
                { labelKey: "home.tradeActivity", val: "+12%", cls: "text-emerald-600" },
                { labelKey: "home.fxVolatility", val: "LOW", cls: "text-emerald-500" },
                { labelKey: "home.supplyStress", val: "MED", cls: "text-amber-500" },
              ].map(({ labelKey, val, cls }) => (
                <div key={labelKey} className="bg-gray-50 rounded-lg p-2 text-center border border-gray-200">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
            <div className="flex items-center gap-1.5 mb-3">
              ...
              <h3 className="text-xs font-bold text-gray-900 uppercase">{t("home.aseanFocus")}</h3>
            </div>
              ...
              {[
                ...
                <div key={title} className="flex items-start gap-2">
                  <span className={clsx(
                    ...
                    tag === "TRADE" ? "bg-emerald-50 text-emerald-700" :
                    tag === "TECH" ? "bg-teal-50 text-teal-700" :
                    tag === "CREDIT" ? "bg-cyan-50 text-cyan-700" :
                    "bg-emerald-50 text-emerald-600"
                  )}>{tag}</span>
                  <p className="text-[11px] text-gray-600 font-medium leading-tight">{title}</p>
                </div>
              ))}
            </div>
          </div>
